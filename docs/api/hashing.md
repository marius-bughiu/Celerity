...
It sits one rung beyond the throughput-oriented peers at the top of the `string` escalation ladder: `StringFnV1AHasher` (cheapest, low-byte only) → `StringFnV1AFullHasher` (cheap, full Unicode width, 32-bit state) → `StringFnV1A64Hasher` (full Unicode width, 64-bit state) → `StringMurmur3Hasher` / `StringXxHash32Hasher` / `StringXxHash64Hasher` / `StringMetroHash64Hasher` / `StringCityHash64Hasher` (strong avalanche, maximum throughput) → `StringSipHash24Hasher` (strong avalanche, **keyed** hash-flooding resistance). Prefer one of the xxHash / MetroHash / CityHash peers when the keys are trusted — they are faster. Reach for `StringSipHash24Hasher` when the keys come from an untrusted source (request paths, header names, user-supplied identifiers) and an adversary could otherwise deliberately drive worst-case collisions into the probe path.
