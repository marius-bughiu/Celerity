...
| `void Insert(int index, char / string / ReadOnlySpan<char>)` | Insert at `index`; `Length` appends. `O(log n + k)` in the document length and the `k` characters inserted — text that does not fit in the target leaf is copied into a fresh run of leaves, so inserting a megabyte costs a megabyte however shallow the tree is. What the tree buys is that the *document* never moves. For the short insertions an editor produces the `k` term vanishes and so does the allocation: when the target leaf has room this is one `Array.Copy` inside it and **nothing is allocated at all**. Inserting nothing is a no-op that does not invalidate enumerators. |
