...
- **No reflection.** No package in the family contains `System.Reflection` usage, no `Activator.CreateInstance`, no `Type.MakeGenericType`, no `System.Linq.Expressions`, and no IL emit. Nothing depends on metadata that the trimmer might remove or that the AOT compiler cannot resolve statically.
...
2. **End-to-end publish smoke test.** [`src/Celerity.AotSmokeTest`](../src/Celerity.AotSmokeTest) is a console app (`<PublishAot>true</PublishAot>`) that drives the library through roughly 2,400 lines and 544 runtime assertions. It references `Celerity.Collections`, `Celerity.Sorting` and `Celerity.Statistics`, picking up `Celerity.Hashing` and `Celerity.Primitives` transitively, and covers:

   - **44 of the 51 collection types** in `Celerity.Collections` — the dictionary and set families, the frozen/perfect-hash pair, `CelerityMultiMap` / `CelerityMultiSet`, `LruCache`, `Deque`, `DisjointSet`, `IndexedPriorityQueue`, `SparseSet`, `CompressedIntSet`, `FenwickTree`, `SegmentTree`, the spatial indexes (`KdTree`, `SpatialGrid`, `RTree`, `IntervalTree`), `TimerWheel`, `CompressedGraph`, the text indexes (`SuffixArray`, `AhoCorasick`, `Trie`), the ordered `BTreeDictionary` / `BTreeSet` / `RankedSet`, `BitSet` / `RankSelectBitVector`, and the probabilistic structures (`BloomFilter`, `CuckooFilter`, `XorFilter`, `HyperLogLog`, `CountMinSketch`). See [Not yet covered](#not-yet-covered) for the seven that are not.
   - **Every hasher in `Celerity.Hashing`** — all 43 of them, across the integer, string, GUID and identity families, plus `DefaultHasher<T>` (which routes through `EqualityComparer<T>.Default`, the most AOT-sensitive path in the library) and the `IHashProvider64<T>` dispatch.
...
Everything below is a gap in the *end-to-end* verification. All eight packages still get the analyzer enforcement described above.

- The **showcase-tier packages** — `Celerity.Ring`, `Celerity.Sentinel` and `Celerity.Cardinality` — are not referenced by the smoke test, so no native binary exercises them. They are built on `Celerity.Collections`, whose instantiations the smoke test does root.
- **Seven collection types** are never constructed by the smoke test: `EnumSet`, `HashCachingSet`, `PooledCeleritySet`, `RobinHoodSet`, `SmallSet`, `SwissSet` and `TopKSketch`. Six are the set-shaped siblings of dictionaries that *are* covered and share their probe and storage code, but the instantiations themselves are not rooted by the publish.
