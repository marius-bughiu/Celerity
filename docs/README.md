...
- [Collections](api/collections.md) — dictionaries (`CelerityDictionary`, `RobinHoodDictionary`, `SwissDictionary`, `HashCachingDictionary`, `PooledCelerityDictionary`, `IntDictionary`, `LongDictionary`, `SmallDictionary`, `EnumMap`, `FrozenCelerityDictionary`), sets (`CeleritySet`, `SwissSet`, `RobinHoodSet`, `HashCachingSet`, `PooledCeleritySet`, `IntSet`, `LongSet`, `SmallSet`, `EnumSet`, `FrozenCeleritySet`, `SparseSet`, `CompressedIntSet`), multi-collections (`CelerityMultiMap`, `CelerityMultiSet`), sorted and order-statistics collections (`BTreeDictionary`, `BTreeSet`, `RankedSet`), text collections (`Trie`, `StringInternTable`, `SuffixArray`, `AhoCorasick`, `Rope`), spatial indexes (`KdTree`, `SpatialGrid`, `RTree`, `IntervalTree`), range aggregates (`FenwickTree`, `SegmentTree`), graphs (`CompressedGraph`), probabilistic / bit collections (`BitSet`, `RankSelectBitVector`, `BloomFilter`, `CuckooFilter`, `XorFilter`, `HyperLogLog`, `CountMinSketch`, `TopKSketch`), caches (`LruCache`, `LfuCache`), sequences (`Deque`), timers (`TimerWheel`), priority queues (`IndexedPriorityQueue`), and union-find (`DisjointSet`).
