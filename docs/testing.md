...
| `Collections/<Type>DifferentialTests.cs` | `RankedSetDifferentialTests` | **Yes**, with one exception — `LfuCacheDifferentialTests` drives a bare `Random`, and `LfuCache`'s CsCheck property lives in the model suite instead | The per-type suites, the largest group. |
...
So: **ask the tree, do not infer from a filename.** Do not conclude a type is uncovered because a file you expected does not exist — that inference is what produced [#416](https://github.com/marius-bughiu/Celerity/issues/416), and it went on to produce several more wrong rosters in the process of fixing it.

```bash
node scripts/check_property_coverage.js --list
```

prints every public collection type in `Celerity.Collections` alongside the suites that reference it and whether it is on the nightly fuzz roster, resolved by reading the source rather than by matching names. Run without `--list` it is a **gate**: CI fails when a collection type is referenced by no per-PR oracle suite, so a new collection that ships without coverage is caught on the pull request that adds it rather than by a later audit. It counts a reference only if it appears in code — a collection is routinely named in a sibling's XML docs, and crediting that as coverage is how a hand-maintained roster gets to be wrong while looking right ([#418](https://github.com/marius-bughiu/Celerity/issues/418)).

Two consequences worth knowing before you add a file. A type the gate cannot see is a type it fails: if you invent a **fifth naming convention**, add its suffix to `SUITE_SUFFIXES` in the script and document it in the table above — a suite the script does not read covers nothing as far as CI is concerned, and the script refuses to run rather than skip a file that looks like a suite and matches no convention. And `Celerity.Fuzz` does **not** satisfy the gate on its own: it is the nightly layer, so a type soaked there but ungated per PR still needs a suite.
...
- **Build-once types** — `CompressedGraph`, `SuffixArray`, `AhoCorasick`, `WaveletTree`, `SparseTable`, `SuccinctTrie` — have no operation sequence. They generate the *input* instead (an edge list, a text, a pattern set), construct the type from it, and reconcile every query against the naive answer computed from the same input.
...
At the time of writing that is four suites in the first shape, fourteen in the second and third together, seven in the fourth and six in the fifth — but check the file rather than the count, which is what the middle column is for.
