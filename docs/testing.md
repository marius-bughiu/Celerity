...
**Not universal, and here is the gap.** Three collections have no property or differential *suite* in either home, so nothing compares them to an oracle on a pull request. They differ in what else backs them up:

- **`EnumMap` and `TopKSketch`** — nothing anywhere. No suite, and no `Celerity.Fuzz` target either, so behavioural tests and the 100% line-and-branch gate are the whole of their coverage. The gate proves every branch executed, not that the result matched an oracle on a sequence nobody wrote down.
- **`XorFilter`** — no per-PR suite, but it does have a nightly fuzz target (`XorFilterCase`, plus `XorFilterHash64Case`). Covered, just not on the pull request that breaks it.

Tracked in [#418](https://github.com/marius-bughiu/Celerity/issues/418).

Everything else in `Celerity.Collections` is covered — but **two suites are named after the operation family rather than the type**, so a search by filename misses them: `SetAlgebraDifferentialTests` carries `SmallSet` (alongside the seven hash-backed sets) and `EnumSetAlgebraDifferentialTests` carries `EnumSet`, which cannot join the `int`-parameterized battery. Search for the type name across `Collections/`, not for a `<Type>DifferentialTests.cs` that may not exist.
