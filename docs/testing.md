...
| Property-based tests | **two homes** — `Celerity.Tests/Properties/` (cross-family model suite) and `Celerity.Tests/Collections/` (`*DifferentialTests.cs` per type or operation family, `*AccuracyTests.cs` for the approximate types) | Over randomized operation sequences or generated inputs, an *exact* collection stays observably equal to its oracle, and an *approximate* one stays inside the guarantee it advertises. Coverage is broad but not universal; the section below says how to check a given type. | `dotnet test` |
...
**How to tell what covers a given type — and why you should check rather than infer.** Randomized oracle coverage lives under at least four naming conventions, and no one of them is the index:
...
| `Collections/<Type>AccuracyTests.cs` | `TopKSketchAccuracyTests`, `CountMinSketchAccuracyTests`, `HyperLogLogAccuracyTests` | Approximate types checked against an exact model — not for equality but for the guarantee each advertises, deterministic for the first two and statistical for `HyperLogLog`. Inputs vary: `TopKSketch` and `CountMinSketch` use fixed-seed random streams, `HyperLogLog` uses fixed sequential and string datasets sized to its error bound. |
