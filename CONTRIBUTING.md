...
- `CopyTo(T[] array, int arrayIndex)` validates through `CopyToGuard.Validate` rather than by hand. The argument contract is `Dictionary<,>.CopyTo`'s — a null array is `ArgumentNullException`, an index that is negative **or past the end of the array** is `ArgumentOutOfRangeException`, and only a valid index with too little room after it is `ArgumentException`. It is stricter than `HashSet<T>.CopyTo`, which reports a past-the-end index as `ArgumentException`; the library answers the dictionary way throughout, sets included. Writing the three checks out per collection is how `Deque<T>` and `PersistentVector<T>` each came to be missing the middle one and to report a bad index as insufficient space ([#440](https://github.com/marius-bughiu/Celerity/issues/440)). `CopyToArgumentGuardTests` fails if a new `CopyTo` overload is not covered.
...
- New collections are expected to carry parity coverage at every layer: behavioural tests, a CsCheck property test against the closest BCL oracle, and a `Celerity.Fuzz` target. See the [Testing & coverage guide](docs/testing.md) for how each layer works and how to run them. Four points the rule leaves implicit:
  - **The property test and the fuzz target are not alternatives.** They share an oracle but not a job: the property test is bounded and runs on every pull request; the fuzz target is unbounded and runs nightly, reaching sequences no per-PR budget can. (Whether a failure *shrinks* depends on how the property is written, not on which layer it is — see [what a failure gives you](docs/testing.md#what-a-failure-gives-you).) A type with only the second one still has its behavioural tests and the coverage gate on every pull request — what it lacks there is the *randomized oracle* comparison, so a regression that no hand-written case happens to cover surfaces the next morning against `main` rather than on the pull request that caused it.
  ...
  - **Never conclude a type is uncovered from a filename search.** Oracle coverage lives under four conventions in the test project — the shared `Celerity.Tests/Properties/CollectionModelPropertyTests.cs`, a per-type `Collections/<Type>DifferentialTests.cs`, a per-*operation* one (`SetAlgebraDifferentialTests` carries `SmallSet`, `EnumSetAlgebraDifferentialTests` carries `EnumSet`, `SketchMergeDifferentialTests` the four merge paths), and `Collections/<Type>AccuracyTests.cs` for the approximate types — plus `Differential.All` for the nightly fuzz layer. `grep -rl "\bYourType\b" src/Celerity.Tests/` rather than guessing at a filename. [#416](https://github.com/marius-bughiu/Celerity/issues/416) was filed because only the first was checked, and the fix for it got the roster wrong five more times the same way; [docs/testing.md](docs/testing.md#property-based-tests-cscheck) has the full table.
  - **Oracle coverage is not the same as a CsCheck property, and only the latter satisfies this rule.** The accuracy suites are ordinary xUnit cases, thirteen of the differential suites drive a bare `Random` rather than CsCheck, and `Differential.All` is the fuzz harness rather than a property at all. A type covered only by one of those still owes the property test. The **Property test?** column in [docs/testing.md](docs/testing.md#property-based-tests-cscheck) says which is which. A new test goes next to its siblings if it has any, and in its own `<Type>DifferentialTests.cs` otherwise.
- A new collection must also be added to the **cross-collection suites** that assert one rule across the whole family, not only to its own `*Tests.cs`. `grep` `src/Celerity.Tests/Collections/` for them before assuming you have found them all; `ClearNoOpVersionTests.cs` (a `Clear()` that removes nothing must not bump the version) is the one that applies to every count-based collection. Its class comment records the types that are deliberately out of scope and why, so a new immutable or build-once collection belongs in that note rather than in the battery. A type that is absent from these suites is not covered for the invariants the rest of the family guarantees.
