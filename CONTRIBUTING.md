...
- `CopyTo(T[] array, int arrayIndex)` validates through `CopyToGuard.Validate` rather than by hand. The argument contract is `HashSet<T>`'s — a null array is `ArgumentNullException`, an index that is negative **or past the end of the array** is `ArgumentOutOfRangeException`, and only a valid index with too little room after it is `ArgumentException`. Writing the three checks out per collection is how `Deque<T>` and `PersistentVector<T>` each came to be missing the middle one and to report a bad index as insufficient space ([#440](https://github.com/marius-bughiu/Celerity/issues/440)). `CopyToArgumentGuardTests` fails if a new `CopyTo` overload is not covered.
