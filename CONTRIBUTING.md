...
- New collections are expected to carry parity coverage at every layer: behavioural tests, a CsCheck property test against the closest BCL oracle, and a `Celerity.Fuzz` target. See the [Testing & coverage guide](docs/testing.md) for how each layer works and how to run them. Three points the rule leaves implicit:
  ...
  - **Never conclude a type is uncovered from a filename search.** Randomized oracle coverage lives under several conventions — the shared `Celerity.Tests/Properties/CollectionModelPropertyTests.cs`, a per-type `Collections/<Type>DifferentialTests.cs`, a per-*operation* one (`SetAlgebraDifferentialTests` carries `SmallSet`), a `Collections/<Type>AccuracyTests.cs` for the probabilistic types, and `Differential.All` for the nightly layer. `grep -rl "\bYourType\b" src/Celerity.Tests/` instead. [#416](https://github.com/marius-bughiu/Celerity/issues/416) was filed because only the first was checked, and the fix for it got the gap list wrong four more times the same way; [docs/testing.md](docs/testing.md#property-based-tests-cscheck) has the full table. A new test goes next to its siblings if it has any, and in its own `<Type>DifferentialTests.cs` otherwise.
