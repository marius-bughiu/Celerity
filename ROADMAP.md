...
- Specialized collections for domain-specific workloads (e.g. graph traversal, spatial indexing). Status: `in-progress` — `BloomFilter<T, THasher>` ships the first specialized type: a probabilistic membership filter with bit-array storage, no false negatives, and a tunable false-positive rate, sizing `m` / `k` from the expected element count and deriving its `k` bit probes from a single `IHashProvider<T>` call by double hashing. Remaining candidates on the issue checklist (`BitSet`, `CountMinSketch`, `HyperLogLog`, …) are still planned. Tracked in [#30](https://github.com/marius-bughiu/Celerity/issues/30).
