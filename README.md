...
- `IntervalTree<TKey, TValue>` — a **build-once index over half-open `[start, end)` ranges** answering *which ranges cover this point* and *which overlap this window* in time that tracks the matches found rather than the intervals stored (`O(log n + k)` when they cluster, `O(min(n, k log n))` when scattered). The BCL has no interval structure at all, so the alternative is a `List<T>` scanned per query — and sorting it by start does not help, because a range that begins far to the left can still cover the point. Booking- and scheduling-conflict checks, IP-range lookup, effective-dated pricing, "which trace spans were live at *t*". The whole structure is four flat arrays with no per-interval node, every query shape shares one traversal specialized per `struct` visitor, and the predicate / count / copy tiers allocate nothing.
...
`IntervalTree<TKey, TValue>` is a build-once index over half-open `[start, end)` ranges. .NET ships nothing for this — no interval tree, no interval map, no range-overlap query anywhere in `System.Collections` — so the alternative is a `List<T>` filtered per query, and keeping that list sorted by start does not help: a range beginning far to the left can still cover the query point, so the scan cannot stop early.
...
| **Which ranges cover this point, or overlap this window** — booking / scheduling conflict checks, IP-range and CIDR-to-owner lookup, effective-dated pricing and feature-flag windows, "which trace spans were live at *t*" | `IntervalTree<TKey, TValue>` | Build-once index over half-open `[start, end)` ranges, costing what the matches cost rather than what the collection costs — `O(log n + k)` when matches cluster, `O(min(n, k log n))` when scattered — against the `O(n)` `List<T>` scan the BCL leaves you with — it ships no interval structure at all, and sorting the list by start does not help, because a range beginning far to the left can still cover the point. At 100k intervals: **151x** on the point query, **79x** on the window query, **27x** on the first-overlap conflict check, and the predicate / count / copy tiers allocate nothing. The ratio tracks **selectivity**, not just size — on a shape ten times denser (~1,250 matches per point) the point query falls to **8.3x**, since the tree still has to visit every match. Build is the cost: **28x** a `List<T>` fill and 2.3x its memory, paid once. Immutable, so adding a range means rebuilding; overlapping ranges stay distinct entries rather than being coalesced. |
