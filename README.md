...
- For `uint` keys, `UInt32Hasher` is the cheap XOR-fold default; escalate to `UInt32Murmur3Hasher` (the Murmur3 `fmix32` finalizer) when the fold produces clustering or you need strong avalanche on adversarial keys, mirroring the `int` family's `Int32Murmur3Hasher` step.
