...
- `SparseTable<T, TMonoid>` — the **build-once** counterpart: the same range fold, answered in **`O(1)`** — two array reads and one combine, no descent. It precomputes the fold of every power-of-two window and covers a range with the two anchored at its ends; those windows *overlap*, so the fold must be **idempotent**, and that law is enforced by the compiler rather than by a doc note — the type takes `IIdempotentMonoid<T>`, which `Min` / `Max` / `BitwiseAnd` / `BitwiseOr` implement and `SumMonoid` deliberately does not, so `SparseTable<int, SumMonoid<int>>` will not build. The query measures **10.9×** a `SegmentTree` over the same sequence at 1k and **18.8×** at 100k (and 122× / 8,200× the array scan), paid for with a **4.9× slower build** and **5–8.5× the memory** — so it wins on a sequence built once and queried past roughly **960** times at 1k, **63,000** at 100k. Immutable, so once built an instance is safe to share for reading — provided the fold is too, since every query calls it.
...
**The build is the price.** `O(n log n)` into `levels × n` cells against a segment tree's `O(n)` into `2n`: 4.9× the build time and 5–8.5× the memory. It repays at roughly 960 queries at 1,000 elements and 63,000 at 100,000 — below that, `SegmentTree` is the better type even on a sequence that never changes.
