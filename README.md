...
**At 1,000 boxes the margin against the hand-roll all but disappears**: 1.35x on the overlap query and 1.12x on the point query. **Build is the price**, paid once: 1.29 ms → 34.77 ms at 100,000 boxes (**27x** the cost of merely copying the array) — a sort per level, which is what "Sort-Tile-Recursive" costs.
...
- **The received wisdom about shape did not survive being measured.** The standard advice is that an R-tree only earns its keep on extents spanning orders of magnitude, and that uniformly sized boxes belong to a bucketed grid. `RTreeShapeBenchmark` in the extended suite gives the grid an arm of its own rather than inferring it, and the tree comes out **1.25x** ahead of the grid on the varying shape and **1.98x** ahead on the uniform one — its widest margin is on the shape it was expected to lose. A grid's query cost is dominated by the cells a query covers rather than the boxes in them, while an R-tree's node boxes get tighter as extents get more alike. The qualification is that a grid's cell size is a tuning knob and this one is sized by the data rather than the query; the real reason to reach for a grid is that it is **mutable** and this type is not. The same benchmark also settles the packing choice: sort-tile is level with a Hilbert order on varying extents and 1.19x ahead on uniform ones.
