...
- `LfuCache<TKey, TValue, THasher>` — fixed-capacity **least-frequently-used cache** with `O(1)` get/put — *worst case*, not amortized — and ties broken by least-recently-used. It covers the failure mode recency cannot see: **an LRU is scan-vulnerable**, so one sequential pass over a capacity's worth of cold keys evicts the whole hot set; here a scan key arrives at frequency 1 and is the next thing evicted. Frequency buckets and their entry chains sit in fixed-size arrays, so the hot path **allocates nothing** — against an idiomatic `Dictionary` + `SortedSet` LFU that is `O(log n)` per operation and allocates a tree node per insert. Frequencies never age, which is the tradeoff: use it for a stable, skewed popularity distribution and keep `LruCache` for recency-dominated ones.
