...
- `SparseMap<TValue>` — bounded-universe **integer-keyed** dictionary (Briggs–Torczon sparse map): a dense key array and a parallel value array, so a lookup is a direct array index and iteration is a contiguous scan over exactly the present entries. `Clear` leaves the key and sparse arrays untouched — for clear-and-rebuild side tables over ids in `[0, N)` (graph traversal, ECS, sweep-line). At 100,000 entries over a 4x universe: lookup **3.1x** `Dictionary<int, V>`, remove **3.4x**, clear-and-rebuild **2.4x** (`Clear` alone ~500x); ⚠️ enumeration is only 1.13x, it allocates **1.70x** (the `O(Universe)` sparse array), and at 1,000 entries the fill is 13% *slower*. The dictionary counterpart of `SparseSet`.
...
`SparseMap<TValue>` is the same idea for **non-negative integer** keys over a universe fixed at construction — the dictionary counterpart of `SparseSet`, and the Briggs–Torczon sparse map: a dense key array, a parallel dense value array, and a sparse index array pointing each present key at its dense slot. Lookup, insert and removal are `O(1)` with no hashing, and the point of the type is what `Dictionary<int, TValue>` can't match: `Clear()` leaves the key and sparse arrays exactly as they are (a stale sparse entry is rejected by the round-trip check), and enumeration is a contiguous scan over exactly the present entries. That is the winning shape for side tables a traversal rebuilds every pass — BFS distances, ECS components, sweep-line state, where at 100,000 entries lookup runs **3.1x** `Dictionary<int, V>`, removal **3.4x** and clear-and-rebuild **2.4x** (the `Clear` itself ~500x, though the refill it precedes is what dominates the 2.4x). The costs are published with the wins: the `O(Universe)` sparse array is **1.70x** `Dictionary`'s allocation at that size, enumeration is only 1.13x (a near-wash — both sides are memory-bound), and at 1,000 entries filling the map is 13% *slower*, because the fixed sparse-array cost has not been amortized yet. Keys are non-negative-only: one outside `[0, Universe)` throws on the write surface and reads as absent on the read surface. And for a `TValue` that holds a reference, `Clear()` also clears the dense value prefix so nothing is retained — `O(Count)`, against `Dictionary<,>`'s whole bucket array.
