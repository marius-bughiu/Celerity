...
- `SparseMap<TValue>` — bounded-universe **integer-keyed** dictionary (Briggs–Torczon sparse map): a dense key array and a parallel value array, so a lookup is a direct array index and iteration is a contiguous scan over exactly the present entries. `Clear` leaves the key and sparse arrays untouched — for clear-and-rebuild side tables over ids in `[0, N)` (graph traversal, ECS, sweep-line). At 100,000 entries over a 4x universe: lookup **3.1x** `Dictionary<int, V>`, remove **3.4x**, clear-and-rebuild **2.4x**; ⚠️ enumeration is only 1.13x, it allocates **1.70x** (the `O(Universe)` sparse array), and at 1,000 entries the fill is 13% *slower*. The dictionary counterpart of `SparseSet`.
