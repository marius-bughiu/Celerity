...
| **Walk a graph, or resolve a dependency order** — build and package ordering, module and import resolution, link / follower / call graphs, reachability and impact analysis | `CompressedGraph` | Build-once compressed-sparse-row graph over dense vertex ids: an offset per vertex and one contiguous target array, so `Neighbors(v)` is a `ReadOnlySpan<int>` slice rather than a `List<int>` object per vertex. .NET ships no graph type of any kind, so the alternative is `Dictionary<int, List<int>>` or `List<int>[]`; both are benchmarked. At 100k vertices and average degree 8: **2.4x** the dictionary form on a full breadth-first traversal (**1.5x** the `List<int>[]` hand-roll), **1.8x** on a topological order, **11x** on the transpose, **3.5x** on build, and **3.6x** less retained memory. **Read the caveat with the numbers**: the win is a memory-hierarchy win, and at 1,000 vertices — where the whole graph fits in cache — the traversal measures at *parity* with the `List<int>[]` hand-roll, so at that scale reach for this type for the transpose and the API rather than the speed. A pre-registered kill criterion (≥3x on the traversal) was **missed at 2.4x** and the type shipped on the BCL-gap limb of the rule instead, which is recorded rather than rounded away. Immutable, so adding an edge means rebuilding; duplicate edges collapse and vertex ids must be dense. No edge weights and no shortest paths. |
