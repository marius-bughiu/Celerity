...
- For `string` keys, `StringFnV1AHasher` is the fast default for ASCII-dominated workloads (it folds only the low byte of each character). For keys with significant non-ASCII content, step up to `StringFnV1AFullHasher`, which folds the full UTF-16 character at the same FNV-1a cost class and so does not collide characters that differ only in their upper byte (e.g. `'A'` / `'Ł'`); escalate further to `StringMurmur3Hasher` when key distribution is clustered or adversarial and you need the stronger `fmix32` avalanche.
