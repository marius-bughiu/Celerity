...
Matches come out in ascending **end** position, longest first among those ending together — not in ascending `Start`, which a single pass cannot produce. Ordinal over UTF-16 code units; duplicates collapse and the empty pattern is rejected; immutable, so changing the pattern set means rebuilding. **The pattern count is what decides**: against a compiled `Regex` alternation of 256 patterns over 100,000 characters this measures 11.4x on membership and on enumerating every match, allocating nothing against 2.4 MB — but against the plain k-`IndexOf` loop it is 3.2x on counting, 1.19x on rejecting absent patterns, and at **eight** patterns the loop wins by 9x. Below roughly fifty patterns, write the loop.
...
| **Find any of many patterns in a text, in one pass** — log and alert scanning, keyword / profanity / brand filters, WAF and IOC rule sets, dictionary tokenizers, DLP scanning | `AhoCorasick` | Build-once **multi-pattern automaton**: every occurrence of every pattern in one left-to-right pass at `O(n + matches)`, independent of how many patterns there are, reporting **overlapping** matches and which pattern each was. `Trie` matches a prefix of the *query* and `SuffixArray` indexes one fixed *text*, so neither can answer this; the BCL's `string.IndexOf` is a single needle per pass and `SearchValues<string>` answers only *where is the first of these*. Against a compiled `Regex` alternation of 256 patterns over 100,000 characters: **11.4x** on membership, **11.4x** on enumerating every match with **0 bytes** allocated against 2.4 MB, and 8.5x on build. ⚠️ **The pattern count decides, and below the crossover this loses**: against the plain k-`IndexOf` loop it is 3.2x on counting and only 1.19x on rejecting absent patterns, and at **eight** patterns that loop wins by 9x — `MemoryExtensions.IndexOf` is vectorized and this pass reads a character at a time. Break-even is around fifty patterns. Ordinal, immutable, and the empty pattern throws rather than matching everywhere. |
