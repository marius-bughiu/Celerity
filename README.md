...
- `SparseMap<TValue>` — bounded-universe **integer-keyed** dictionary (Briggs–Torczon sparse map): a dense key array and a parallel value array, so a lookup is a direct array index and iteration is a contiguous scan over exactly the present entries. `Clear` leaves the key and sparse arrays untouched — for clear-and-rebuild side tables over ids in `[0, N)` (graph traversal, ECS, sweep-line). At 100,000 entries over a 4x universe: lookup **3.1x** `Dictionary<int, V>`, remove **3.6x**, clear-and-rebuild **2.4x** (`Clear` alone ~500x), fill **2.0x**; ⚠️ enumeration is only 1.13x, it allocates 1.10x (the `O(Universe)` sparse array), and at 1,000 entries the fill is 17% *slower*. The dictionary counterpart of `SparseSet`.
...
- `PersistentHashMap<TKey, TValue, THasher>` — **immutable** hash map over a **CHAMP** trie (the refinement of Clojure's HAMT that Scala's `HashMap` adopted in 2.13): a single-key edit returns a new map **sharing** all but one root-to-leaf path of the old one's storage, a write that changes nothing hands back the receiver, and the bulk methods reach as many branches as their keys do. Branching factor 32 with entries inline in flat arrays and a slot found by popcount, against `ImmutableDictionary<,>`'s AVL tree of branching factor two with a heap node per entry — so a lookup is a few array reads rather than a pointer chase per level. For state read far more often than it is written and still handed to readers as a snapshot. Implements `IReadOnlyDictionary<TKey, TValue?>`.
...
- `PersistentHashSet<T, THasher>` — **immutable** hash set: `PersistentHashMap`'s CHAMP trie with the values taken out. A single-element edit returns a new set **sharing** all but one root-to-leaf path of the old one's storage, an edit that changes nothing hands back the receiver, and `Union` / `Except` / `Intersect` / `SymmetricExcept` run through a builder. Against `ImmutableHashSet<T>`'s AVL tree of branching factor two with a heap node per distinct hash, a membership test is a few popcount-indexed array reads rather than a pointer chase per level. For membership state tested far more often than it is written and still handed to readers as a snapshot. Implements `IReadOnlySet<T>`.
...
- `LfuCache<TKey, TValue, THasher>` — fixed-capacity **least-frequently-used cache** with expected-`O(1)` get/put and ties broken by least-recently-used. Its *eviction bookkeeping* is `O(1)` **worst case** rather than logarithmic; the open-addressed key-index probe is what leaves the end-to-end bound expected, as everywhere else in the library. It covers the failure mode recency cannot see: **an LRU is scan-vulnerable**, so one sequential pass over a capacity's worth of cold keys evicts the whole hot set. Here a scan key arrives at frequency 1, so **a scan of any length costs at most one entry that has been read more than once**. Entries still at frequency 1 are not protected: they sit at the scan's own frequency and are older, so a scan takes them. That is the policy, not a gap in it — an entry used exactly once has demonstrated no reuse to protect. Frequency buckets and their entry chains sit in fixed-size arrays, so the hot path **allocates nothing** — against an idiomatic `Dictionary` + `SortedSet` LFU that is `O(log n)` per operation and allocates a tree node per insert. Frequencies never age, which is the tradeoff: use it for a stable, skewed popularity distribution and keep `LruCache` for recency-dominated ones.
...
- `PersistentVector<T>` — **immutable indexed sequence** over a **32-way bit-partitioned trie** with a tail buffer: every operation returns a new vector **sharing** all but `O(log32 n)` of the old one's storage, and both `this[int]` and `Add` are at most seven array hops — `Add` writes only the tail on 31 appends out of 32, and path-copies `O(log32 n)` nodes on the 32nd. The structure .NET does not ship, between the two that do — `ImmutableArray<T>` copies the whole array on every append, `ImmutableList<T>` is an AVL tree with a heap node per element and pays a pointer chase per level on **every read**. Concurrent readers need no synchronization, as for the build-once types — the difference is that this one is never mutated at all, so an edit produces another vector rather than requiring a rebuild. Implements `IReadOnlyList<T>`.
...
- `SuccinctTrie<TValue>` — the **build-once** prefix tree, whose **tree shape costs two bits per node** (about 3 bytes a node once labels, the terminal bit and the rank/select indexes are counted): the same `GetByPrefix` / `TryGetLongestPrefix` / ordered-enumeration surface as `Trie<TValue>`, over a LOUDS bit vector plus a label array rather than a graph of node objects. This is the composition `RankSelectBitVector`'s own docs name and the library never shipped. What it buys is **footprint**: at 100,000 keys it retains **1.05 MB against `Trie`'s 40.16 MB — 38.1x smaller** — and it never stores the key strings at all, which a `Dictionary<string, TValue>` must. ⚠️ **What it costs is query time, on every arm**: exact `Lookup` is 28x a `Dictionary` and 6.4x `Trie`, and building it is 12.5x. The prefix win is the trie win and it is a matter of *selectivity*: on a nearly-complete token with a handful of completions it is **1,042x** a `Dictionary` (which must scan every key), but on bulk enumeration of a sixteenth of the table it is 3.6x *slower* than one — and so, measurably, is `Trie` — because the dictionary's scan hits a match every sixteen entries and either trie is charged for materializing every result string. Immutable, so a changed key set means a rebuild. Implements `IReadOnlyDictionary<string, TValue?>`.
...
- The same span-keyed probes ship on `FrozenCelerityDictionary`, `FrozenCeleritySet`, `CelerityDictionary<string, …>`, `CeleritySet<string, …>`, `Trie<TValue>`, and `SuccinctTrie<TValue>` — so a tokenizer, CSV/log reader, or route dispatcher holding a slice of its input buffer never has to call `new string(span)` per lookup. Works on all three target frameworks, including the `net8.0` floor where the BCL has no equivalent (.NET 9 added `Dictionary<string,V>.GetAlternateLookup`). See [span-keyed lookups](docs/api/collections.md#span-keyed-lookups).
...
- `SparseTable<T, TMonoid>` — the **build-once** counterpart: the same range fold, answered in **`O(1)`** — two array reads and one combine, no descent. It precomputes the fold of every power-of-two window and covers a range with the two anchored at its ends; those windows *always overlap* — completely, when the range length is itself a power of two — so the fold must be **idempotent**, and that law is enforced by the compiler rather than by a doc note — the type takes `IIdempotentMonoid<T>`, which `Min` / `Max` / `BitwiseAnd` / `BitwiseOr` implement and `SumMonoid` deliberately does not, so `SparseTable<int, SumMonoid<int>>` will not build. The query measures roughly **9–11×** a `SegmentTree` over the same sequence at 1k and **16–19×** at 100k (and ~100× / ~8,200× the array scan), paid for with a **~4–5× slower build** and **5–8.5× the memory** — so it wins on a sequence built once and queried past roughly **800–1,000** times at 1k, **65,000** at 100k. Immutable, so once built an instance is safe to share for reading — provided the fold is too, since every query calls it.
- `WaveletTree` — an **immutable** succinct index over a fixed sequence of `int` values that answers the questions a range fold cannot: the **`k`-th smallest value inside a positional window** (`Quantile`), how many values in that window fall in a **value band** (`RangeCount`), and the rank of a single value — each `O(log σ)` over the *distinct* values, independent of how wide the window is: one descent for `Quantile` and `Rank`, two for `RangeCount` (one per band edge). `Select` is the one query outside that bound — it binary-searches `Rank`, so `O(log n · log σ)`. A `SegmentTree` folds a window down to one value and so structurally cannot reach the `k`-th; a `RankedSet` ranks a whole set with no positional window and no duplicates; the BCL has neither. The baseline is what a caller writes by hand — `Array.Sort` over a copy of the window for a quantile, a counting loop for a band — and at a million values with a 1% window the quantile is **~3,100× faster** and the band count **~45×**. Build-once, and it *loses* to a plain scan on a short window; both are measured in [the API reference](docs/api/collections.md#wavelettree).


**Spatial & interval indexes**
...
- `RangeMap<TKey, TValue>` — a **mutable map from disjoint half-open ranges** of keys to values: the coalescing interval map `IntervalTree` is not. Assigning a value to `[start, end)` overwrites whatever was there, splitting any range that straddles either edge, and ranges left adjacent with equal values merge, so the map always holds the maximal runs. The BCL has nothing for it; the alternative is a `List<T>` kept sorted by start, which binary-searches a lookup well but memmoves its tail on every assignment. Keyspace and IP-range ownership edited as shards move, an allocator's used-versus-free map, effective-dated configuration edited in place, style runs. A B-tree keyed by each range's end, so a lookup is one descent and an assignment `O((k + 1) log n)` for the `k` ranges it overwrites.
...
<details>
<summary><b>Specialized dictionaries</b> — RobinHood, Swiss, HashCaching, Pooled, Frozen, MultiMap, MultiSet, Small, EnumMap, SparseMap</summary>
...
`SparseMap<TValue>` is the same idea for **non-negative integer** keys over a universe fixed at construction — the dictionary counterpart of `SparseSet`, and the Briggs–Torczon sparse map: a dense key array, a parallel dense value array, and a sparse index array pointing each present key at its dense slot. Lookup, insert and removal are `O(1)` with no hashing, and the point of the type is what `Dictionary<int, TValue>` can't match: `Clear()` leaves the key and sparse arrays exactly as they are (a stale sparse entry is rejected by the round-trip check), and enumeration is a contiguous scan over exactly the present entries. That is the winning shape for side tables a traversal rebuilds every pass — BFS distances, ECS components, sweep-line state, where at 100,000 entries lookup runs **3.1x** `Dictionary<int, V>`, removal **3.6x** and clear-and-rebuild **2.4x** (the `Clear` itself ~500x, though the refill it precedes is what dominates the 2.4x). The costs are published with the wins: the `O(Universe)` sparse array puts allocation at 1.10x `Dictionary`'s, enumeration is only 1.13x (a near-wash — both sides are memory-bound), and at 1,000 entries filling the map is 17% *slower*, because the fixed sparse-array cost has not been amortized yet. Pre-size with `EnsureCapacity` before a bulk fill — the universe constructor sizes only the sparse index array. Keys are non-negative-only: one outside `[0, Universe)` throws on the write surface and reads as absent on the read surface. And for a `TValue` that holds a reference, `Clear()` also clears the dense value prefix so nothing is retained — `O(Count)`, against `Dictionary<,>`'s whole bucket array.

```csharp
var distance = new SparseMap<int>(nodeCount);   // universe = ids in [0, nodeCount)

distance.Clear();                                // key + sparse arrays untouched
distance[start] = 0;
...
<details>
<summary><b>Caches</b> — LruCache, LfuCache</summary>
...
`LfuCache` is the frequency-ordered sibling, and it exists for the one failure mode recency cannot see: an **LRU is scan-vulnerable**, so a single sequential pass over a capacity's worth of cold keys evicts the entire hot set no matter how often those keys were used — the ordinary shape of a table scan or a backfill sharing a cache with steady-state traffic. Under LFU a scan key arrives at frequency 1, so **a scan of any length costs at most one entry that has been read more than once** — one if the cache was full with nothing at frequency 1 when the scan began, none if it had spare room. The guarantee is conditional and the condition matters: an entry inserted and never read again sits at the scan's own frequency and is older than the incoming keys, so the scan takes it. LFU protects demonstrated reuse; a single use demonstrates none. Against an LRU, which loses its whole working set to the same scan however hot it was. Entries sit in **frequency buckets** held in an ascending-frequency list, both threaded through fixed-size arrays, which makes the eviction bookkeeping `O(1)` *worst case* and allocation-free — against an idiomatic `Dictionary` + `SortedSet` LFU that is `O(log n)` and allocates a tree node per insert. (A whole operation is expected `O(1)`: it also probes the open-addressed key index, like every hash-backed type here.) The tradeoff is stated rather than hidden: **frequencies never age**, so a key that was hot long ago can hold its slot. Use `LfuCache` for a stable, skewed popularity distribution; use `LruCache` when what matters is what was touched recently. See [the API reference](docs/api/collections.md#lfucachetkey-tvalue-thasher).

```csharp
var cache = new LfuCache<long, string, Int64WangHasher>(capacity: 3);
cache[1] = "one";
cache[2] = "two";
_ = cache[1];             // 1 is now at frequency 2; 2 is still at 1
cache[3] = "three";       // full
cache[4] = "four";        // evicts the least-frequently-used (2), not 1
Console.WriteLine(cache.ContainsKey(2)); // False (evicted)
Console.WriteLine(cache.ContainsKey(1)); // True  (spared by its frequency)

cache.TryGetFrequency(1, out long uses);
Console.WriteLine(uses);                 // 2 — the insert plus the read
```

</details>

<details>
<summary><b>Sequences</b> — Deque, PersistentVector</summary>
...
`PersistentVector<T>` is the **immutable** sequence: a 32-way bit-partitioned trie with a tail buffer, where every operation returns a new vector sharing all but one root-to-leaf path with the old one. .NET ships the two ends of this trade and nothing between them — `ImmutableArray<T>` indexes like an array but copies the whole array on every append (`O(n²)` to build), and `ImmutableList<T>` appends in `O(log n)` but is an AVL tree with a heap node per element, so it pays a pointer chase per level on every read too. At 100,000 elements this type indexes **14.3x** faster than `ImmutableList<T>`, appends **16.3x** faster and retains **9.4x** less memory. See [the API reference](docs/api/collections.md#persistentvectort).

```csharp
PersistentVector<string> log = PersistentVector<string>.Empty;
log = log.Add("started").Add("connected");

PersistentVector<string> snapshot = log;      // hand to a reader on another thread — no copy
log = log.Add("closed");                      // snapshot still has 2; log has 3

string second = log[1];                       // "connected" — a handful of array hops
PersistentVector<string> amended = log.SetItem(0, "restarted"); // log is untouched
```

</details>

<details>
<summary><b>An immutable map</b> — PersistentHashMap</summary>

`PersistentHashMap<TKey, TValue, THasher>` is the **immutable** map, and the associative half of the lane `PersistentVector<T>` opened. .NET ships exactly one — `ImmutableDictionary<TKey, TValue>` — and it is an **AVL tree keyed by hash code** with one heap node per entry carrying key, value, hash, two children and a height field, so its branching factor is two and every lookup is a pointer chase per level over nodes scattered across the heap. This is a **CHAMP** trie (Steindorfer & Vinju, OOPSLA 2015 — the refinement of the hash-array mapped trie behind Clojure's `PersistentHashMap`, which interleaves entries and sub-nodes under one bitmap; CHAMP is what Scala's `HashMap` adopted in 2.13): a node reads five bits of the hash, so the branching factor is 32 and 100,000 entries are four levels deep rather than seventeen, with entries inline in flat arrays and the slot found by a popcount over a 32-bit occupancy map.

Reach for it when a map is **read far more often than it is written but still has to change** — a config or feature-flag set swapped atomically and read on every request, a symbol table threaded through a compiler pass, an interpreter environment, per-version state handed to readers on another thread. At 100,000 entries it looks up **10.6x** faster than `ImmutableDictionary<int, string>`, inserts **2.1x** faster and retains about **20% less** memory. Memory is the weakest of those figures and is stated as such in the reference: the read is the reason to reach for this type. If the map never changes after it is built, `FrozenCelerityDictionary` or a plain `Dictionary<,>` is faster still; if nobody holds an old version, structural sharing buys nothing and `CelerityDictionary` writes one slot where this allocates a path. See [the API reference](docs/api/collections.md#persistenthashmaptkey-tvalue-thasher).
...
flags = flags.SetItem("new-checkout", true);  // snapshot still reads false

bool live = flags["new-checkout"];            // True
bool asPublished = snapshot["new-checkout"];  // False
```

</details>

<details>
<summary><b>An immutable set</b> — PersistentHashSet</summary>

`PersistentHashSet<T, THasher>` is the set half of `PersistentHashMap`: the same **CHAMP** trie with the value array taken out. .NET's only immutable hash set, `ImmutableHashSet<T>`, is an **AVL tree keyed by hash code** with a heap node per distinct hash, so its branching factor is two and every membership test is a pointer chase per level; here a node reads five bits of the hash, 100,000 elements are four levels deep, and elements sit inline in one flat array per node. `Union` / `Except` / `Intersect` / `SymmetricExcept` return new sets and hand back the receiver when nothing changed, and the `IReadOnlySet<T>` queries follow `HashSet<T>`'s semantics.

Reach for it when a set is **tested far more often than it is written but still has to change** — an allow- or deny-list swapped atomically and checked on every request, a visited set threaded through a backtracking search, the names in scope in a compiler pass. At 100,000 elements it answers `Contains` **5.2x** faster than `ImmutableHashSet<int>`, adds **2.4x** faster and retains **1.71x** less memory, though `Remove` allocates about 16% more while running 2.1x faster. If the set never changes after it is built, `FrozenCeleritySet` or a plain `HashSet<T>` is faster still; if nobody holds an old version, `CeleritySet` writes one slot where this allocates a path. See [the API reference](docs/api/collections.md#persistenthashsett-thasher).
...
blocked = blocked.Add("192.0.2.99");          // snapshot is untouched

bool live = blocked.Contains("192.0.2.99");   // True
bool asPublished = snapshot.Contains("192.0.2.99"); // False
```
...
<details>
<summary><b>The same fold in O(1), once the sequence stops changing</b> — SparseTable</summary>

`SparseTable<T, TMonoid>` is the **build-once** counterpart to `SegmentTree`. It precomputes the fold of every window whose length is a power of two, then answers an arbitrary range from the two such windows anchored at its ends — **`O(1)`**, two array reads and one combine, whatever the width. Those windows *always overlap* — by `2·2ᵏ - length` elements, which is never zero, and at an exact power of two they are the *same* window — so the fold must be **idempotent** (`Combine(a, a) == a`), and that is a constraint rather than a warning: the type takes `IIdempotentMonoid<T>`.

```csharp
// A finished run's latency history: fixed once the run ends, questioned repeatedly afterwards.
long[] latencies = { 105, 102, 108, 101, 110, 103 };
var history = new SparseTable<long, MinMonoid<long>>(latencies);

Console.WriteLine(history.Query(0, 4));   // 101 — two array reads, no descent
Console.WriteLine(history.Query(1, 6));   // 101 — a five-wide window costs the same as a two-wide one
Console.WriteLine(history.Aggregate);     // 101 — O(1), where SegmentTree.Aggregate is O(log n)

// Sum is not idempotent, so a sparse table would double-count the overlap. The compiler says so:
// var wrong = new SparseTable<long, SumMonoid<long>>(latencies);   // does not compile

// The gcd fold from the SegmentTree example qualifies — gcd(a, a) == a — so it only needs the
// derived interface: `public readonly struct GcdMonoid : IIdempotentMonoid<uint>`.
```

**The build is the price.** `O(n log n)` into `levels × n` cells against a segment tree's `O(n)` into `2n`: about 4–5× the build time and 5–8.5× the memory. It repays at roughly 800–1,000 queries at 1,000 elements and 65,000 at 100,000 — below that, `SegmentTree` is the better type even on a sequence that never changes.

</details>

<details>
<summary><b>The k-th smallest value inside a window</b> — WaveletTree</summary>

`WaveletTree` is the build-once index over a sequence of `int` values that reaches the *value* axis a range fold collapses. `SegmentTree<long, MinMonoid<long>>` answers the smallest value in a window; nothing in the library — or in the BCL — could answer the **median** of one, because a monoid fold has no way to keep the values it discarded.

```csharp
// A million request latencies in arrival order, indexed once after the run.
var index = new WaveletTree(latencies);

// The median and p99 of requests 400,000 through 500,000. No sort, no copy, and the
// cost does not grow with the width of the window.
Console.WriteLine(index.Quantile(400_000, 100_000, 50_000));   // median
Console.WriteLine(index.Quantile(400_000, 100_000, 99_000));   // p99

// How many of those breached a 250 ms SLO, and how often exactly 200 ms was recorded.
Console.WriteLine(index.RangeCount(400_000, 100_000, 251, int.MaxValue));
Console.WriteLine(index.Rank(index.Length, 200));
```

The values are coordinate-compressed, so the cost follows the number of *distinct* values rather than their magnitude: a thousand distinct latencies cost ten levels whatever the milliseconds are. It is build-once — changing a value means rebuilding — and on a short window the fixed ten-level descent loses to the loop it replaces.
...
Intervals are half-open, so `[0, 10)` and `[10, 20)` tile without conflicting. Overlapping ranges stay distinct entries — this is not a coalescing interval map, which is `RangeMap` below — and the tree is immutable, so adding a range means building a new one.

</details>

<details>
<summary><b>Map ranges of keys to values, and keep editing them</b> — RangeMap</summary>

`RangeMap<TKey, TValue>` maps **disjoint half-open ranges** to values. Assigning a value to `[start, end)` overwrites whatever was there, splitting a range that straddles either edge, and ranges left adjacent with equal values merge — so the map always holds the maximal runs, and `Count` is how many there are. .NET ships nothing for this; the hand-roll is a `List<T>` sorted by start, which binary-searches a lookup well but memmoves its tail on every assignment.

```csharp
// Who owns each slice of a keyspace, edited as shards move.
var owners = new RangeMap<int, string>();
owners.Set(0, 1024, "node-a");
owners.Set(256, 512, "node-b");        // splits node-a's range in two
owners.Set(512, 768, "node-b");        // merges with the adjacent node-b range

owners[300];                           // "node-b"
owners.Count;                          // 3
owners.TryGetRange(300, out var run);  // [256, 768) = node-b

owners.Remove(700, 800);               // trims the straddled ranges rather than dropping them
foreach (var range in owners.EnumerateOverlapping(200, 300))
    Console.WriteLine(range);          // [0, 256) = node-a, [256, 700) = node-b
```

A write that changes nothing — assigning a value a single range already holds, removing an unmapped range — leaves the map and its enumerators untouched. It is for maps you keep **editing**: if the ranges are built once and only read, a sorted array's binary search is the faster lookup.
...
| Dictionary keyed by small **non-negative ints** over a bounded range whose contents are **rebuilt often** — per-traversal side tables (BFS distance / parent / colour), ECS component storage, sweep-line state | `SparseMap<TValue>` | Briggs–Torczon sparse map (dense key array + parallel dense value array + sparse index array): lookup / insert / remove are `O(1)` with no hashing, `Clear` leaves the key and sparse arrays untouched (vs `Dictionary<int, V>` zeroing its whole entry table), and enumeration is a contiguous scan over just the present entries. At 100,000 entries over a 4x universe: lookup **3.1x**, remove **3.6x**, clear-and-rebuild **2.4x**, fill **2.0x**. Costs `O(Universe)` memory (1.10x `Dictionary`'s allocation when pre-sized with `EnsureCapacity`) and stores only keys in `[0, Universe)`; ⚠️ enumeration is only 1.13x and at 1,000 entries the fill is 17% *slower*, so this is for larger, hotter, repeatedly-rebuilt maps — for an unbounded or huge-and-sparse key space use `IntDictionary` / `Dictionary<int, V>`. For a `TValue` holding a reference, `Clear` also clears the dense value prefix (`O(Count)`) so no value is retained. The dictionary counterpart of `SparseSet`. |
...
| **Bounded cache that must survive a scan** — steady-state traffic sharing a cache with a batch job, backfill, or crawler that sweeps every key once; or any workload with a **stable, skewed** popularity distribution (a Zipfian read cache) | `LfuCache<TKey, TValue, THasher>` | Fixed-capacity least-frequently-used cache: expected-`O(1)` get/put, ties broken by least-recently-used. The eviction bookkeeping is **worst-case** `O(1)` — logarithmic in a sorted-structure LFU — while the open-addressed key-index probe leaves the end-to-end bound expected, as everywhere else here. Solves `LruCache`'s one blind spot — a single pass over `Capacity` cold keys evicts an LRU's entire hot set, whereas here each scan key enters at frequency 1, so **a scan of any length costs at most one entry that has been read more than once**. Note the condition: entries still at frequency 1 sit at the scan's own frequency and are older, so a scan evicts them too — this protects *demonstrated reuse*, not mere residency. Frequency buckets over fixed-size arrays, so the hot path **allocates nothing**, against an idiomatic `Dictionary` + `SortedSet` LFU at `O(log n)` with a tree node per insert. **Frequencies never age**, so a once-hot key can hold its slot — prefer `LruCache` when recency is what matters, and this when popularity is. Single-threaded, for the same reason: reads count as uses and mutate the structure. |
...
| **An immutable sequence you keep appending to** — an append-only event or audit log, a parsed token stream, an undo history, a document model versioned per edit, or any snapshot handed to readers on another thread | `PersistentVector<T>` | Immutable indexed sequence over a **32-way bit-partitioned trie** with a tail buffer: `Add` writes only the tail on 31 appends out of 32 and path-copies `O(log32 n)` nodes on the 32nd, `this[int]` is at most seven array hops, and every result **shares** all but one root-to-leaf path with its receiver. .NET ships only the two ends of the trade — `ImmutableArray<T>` copies the whole array per append (`O(n²)` to build) and `ImmutableList<T>` is an AVL tree with a heap node per element, so `this[int]` chases pointers per level. **14.3x** its indexed read, **16.3x** its append and **9.4x** less retained memory at 100,000 elements. If the sequence never grows, `ImmutableArray<T>` indexes faster still; if you insert in the middle, `ImmutableList<T>` does that in `O(log n)` and a vector cannot. |
| **An immutable map you keep editing** — a config or feature-flag set swapped atomically and read on every request, a symbol table threaded through a compiler pass, an interpreter environment, or any per-version state handed to readers on another thread | `PersistentHashMap<TKey, TValue, THasher>` | Immutable hash map over a **CHAMP** trie: branching factor 32 with entries inline in flat arrays and the slot found by a popcount over a 32-bit occupancy map, so a lookup is a few array reads and a single-key edit **shares** all but one root-to-leaf path with its receiver (the bulk methods touch as many branches as their keys reach, and run through a builder for that reason). .NET's only immutable map, `ImmutableDictionary<,>`, is an AVL tree keyed by hash with a heap node per entry and branching factor two, so a lookup chases a pointer per level — about seventeen at 100,000 entries. **10.6x** its lookup and **2.1x** its insert at 100,000 entries, with about **20% less** retained memory — the read is what earns the type, not the footprint. If the map never changes once built, `FrozenCelerityDictionary` or `Dictionary<,>` is faster; if nobody holds an old version, `CelerityDictionary` writes one slot where this allocates a path; for ordered iteration or range scans use `BTreeDictionary`. |
| **An immutable set you keep editing** — an allow- or deny-list swapped atomically and checked on every request, a visited set threaded through a backtracking search, the names in scope in a compiler pass, or any per-version membership state handed to readers on another thread | `PersistentHashSet<T, THasher>` | `PersistentHashMap`'s CHAMP trie without the values: branching factor 32, elements inline in flat arrays and the slot found by a popcount, so a membership test is a few array reads and a single-element edit **shares** all but one root-to-leaf path with its receiver (set algebra touches as many branches as its operands reach, and runs through a builder). .NET's only immutable hash set, `ImmutableHashSet<T>`, is an AVL tree keyed by hash with a heap node per distinct hash and branching factor two. **5.2x** its `Contains`, **2.4x** its `Add` and **1.71x** less retained memory at 100,000 elements; `Remove` is 2.1x faster but allocates about 16% more. If the set never changes after it is built, `FrozenCeleritySet` (strings) or `HashSet<T>` is faster; if nobody holds an old version, `CeleritySet` writes one slot where this allocates a path. |
...
| **The same prefix questions over a key set that never changes**, where the index is large and its **memory** is what hurts — a shipped routing or tokenizer dictionary, an autocomplete corpus, a static allow/deny list, a prefix index held per tenant or per shard | `SuccinctTrie<TValue>` | The build-once trie, whose **tree shape costs two bits per node**: a LOUDS bit vector over `RankSelectBitVector` plus one label array, with the key strings never stored at all. At 100,000 keys it retains **1.05 MB against `Trie`'s 40.16 MB (38.1x smaller)**, and answers the same `GetByPrefix` / `TryGetLongestPrefix` / ordered enumeration. ⚠️ **Every query arm is slower** — exact `Lookup` 28x a `Dictionary` and 6.4x `Trie`, build 12.5x — and the prefix win depends on *selectivity*: **1,042x** a `Dictionary` on a nearly-complete token with few completions, but 3.6x *slower* on bulk-enumerating a sixteenth of the table (an arm `Trie` does not win either). Immutable: a changed key set means a rebuild, so fill a `Trie` and snapshot it. Implements `IReadOnlyDictionary<string, TValue?>`; thread-safe to share. |
...
| **Range min / max / gcd / bit-mask over a sequence you keep mutating** — sliding-window extrema over a live history, "cheapest offer in this price band" over an order book, per-window capability masks, or any other **associative fold with no inverse** | `SegmentTree<T, TMonoid>` | Point update and range query both `O(log n)`, in one flat array of `2n` cells. `FenwickTree<T>` cannot answer these at all: it computes a range as the *difference* of two prefix folds, so the operation must be invertible. Five folds ship (`Sum` / `Min` / `Max` / `BitwiseAnd` / `BitwiseOr`) and any associative one you write is a field-free struct; non-commutative folds are safe. The BCL has no range-aggregate structure, so the alternative is an `O(n)` scan per query — **14.8× faster** on interleaved update + range-min at 100k, **81×** on a query batch, but only **1.4×** at 1k, where scanning a contiguous array is cache-friendly. If the fold is addition use `FenwickTree<T>` (half the memory); if the sequence never changes after build, `SparseTable<T, TMonoid>` answers any idempotent fold in `O(1)` and a prefix array answers sums in `O(1)`. Range *updates* are not supported — that needs lazy propagation, a different contract. |
| **The same range fold over a sequence that never changes after you build it** — range minima over a finished run's latency history, per-window capability masks over a fixed config, range gcd, a book snapshot replaced rather than edited | `SparseTable<T, TMonoid>` | Range query in **`O(1)`** — two array reads and one combine, whatever the window's width — against `SegmentTree`'s `O(log n)` descent. It covers a range with two power-of-two windows that *always* overlap — completely, at an exact power-of-two length — so the fold must be **idempotent**: `Min` / `Max` / `BitwiseAnd` / `BitwiseOr` and any gcd-like fold you write qualify, and `SumMonoid` does not — the constraint is `IIdempotentMonoid<T>`, so the wrong fold is a compile error rather than a silently doubled overlap. About **100×** the array scan at 1k and **8,200×** at 100k; roughly **9–11×** and **16–19×** a `SegmentTree` over the same values (ranges, because two short runs of identical code disagree by up to 25% — see [the API reference](docs/api/collections.md#what-it-costs)). **Read the price with the number**: the build is `O(n log n)` into `levels × n` cells — **~4–5× slower** than a segment tree's and **5–8.5×** its memory (13.6 MB at 100k) — so it repays at roughly **800–1,000** queries at 1k and **65,000** at 100k, and below that the segment tree wins even on static data. Immutable: no point update, no `Clear`, and safe to share for reading once built, as long as the fold is thread-safe too (the five shipped ones are stateless).
| **The k-th smallest value inside a positional window, or how many values in it fall in a band** — the median / p95 of requests 400k–500k of a run, how many samples in a slice breached a threshold, how often one value occurs in a stretch (latency and metric histories, column-store analytics, sensor logs, any fixed sequence you slice by position and question by value) | `WaveletTree` | **Immutable** succinct index over a sequence of `int` values, built on one `RankSelectBitVector` per level. `Quantile`, `RangeCount` and `Rank` are `O(log σ)` over the *distinct* values, independent of the window width — one descent each, two for `RangeCount`, while `Select` binary-searches `Rank` at `O(log n · log σ)` — a `SegmentTree` folds a window to one value and so cannot reach the k-th, a `RankedSet` has no window and no duplicates, and the BCL has neither. The baseline is `Array.Sort` over a copy of the window (quantile) or a counting loop (band): at a million values with a 1% window that is ~3,100× and ~45×. **Build-once** — changing a value means rebuilding — and on a *short* window the fixed descent loses to the scan. |
...
| **Map ranges of keys to values, and keep editing them** — keyspace and IP-range ownership moved between shards, an allocator's used/free map, effective-dated configuration edited in place, style runs, availability calendars | `RangeMap<TKey, TValue>` | Mutable map from disjoint half-open ranges: an assignment overwrites, splits straddlers at both edges and merges equal neighbours, so the map always holds the maximal runs. The BCL has none; the hand-roll is a `List<T>` sorted by start and patched in place. At 100k ranges an assignment is **7.5x** faster than that list (an `O(n)` memmove against `O((k + 1) log n)`); every read loses to its binary search — lookup **1.5x** slower, a window walk about **3x** — and at 1k ranges the list wins the write too. For maps you edit, not ones built once and read. |
