...
- `LfuCache<TKey, TValue, THasher>` — fixed-capacity **least-frequently-used cache** with expected-`O(1)` get/put and ties broken by least-recently-used. Its *eviction bookkeeping* is `O(1)` **worst case** rather than logarithmic; the open-addressed key-index probe is what leaves the end-to-end bound expected, as everywhere else in the library. It covers the failure mode recency cannot see: **an LRU is scan-vulnerable**, so one sequential pass over a capacity's worth of cold keys evicts the whole hot set; here a scan key arrives at frequency 1 and is the next thing evicted, so a scan of any length costs **at most one** resident entry (one if the cache was already full when it began, none if it had spare room). Frequency buckets and their entry chains sit in fixed-size arrays, so the hot path **allocates nothing** — against an idiomatic `Dictionary` + `SortedSet` LFU that is `O(log n)` per operation and allocates a tree node per insert. Frequencies never age, which is the tradeoff: use it for a stable, skewed popularity distribution and keep `LruCache` for recency-dominated ones.
