...
| **Walk a graph, or resolve a dependency order** — build and package ordering, module and import resolution, link / follower / call graphs, reachability and impact analysis | `CompressedGraph` | Build-once compressed-sparse-row graph over dense vertex ids: an offset per vertex and one contiguous target array, so `Neighbors(v)` is a `ReadOnlySpan<int>` slice rather than a `List<int>` object per vertex. .NET ships no graph type of any kind, so the alternative is `Dictionary<int, List<int>>` or `List<int>[]`; both are benchmarked. At 100k vertices and average degree 8 the traversal is 2.3x the dictionary form and 1.5x a `List<int>[]` — but only **1.06x** an `int[][]` sized exactly and filled in vertex order, and at 1,000 vertices that hand-roll *beats* it. **Traversal speed is not the reason to reach for this.** The reasons are the transpose (**11x**, since the jagged form must allocate a list per vertex to build one), build (3.5x), retained memory (**3.6x** less), and not writing or maintaining the breadth-first order, Kahn's algorithm, the transpose and the sorted-target invariant — none of which the BCL ships. A pre-registered kill criterion (≥3x on the traversal) was **missed at 2.3x**; the type ships on the BCL-gap limb of the rule instead, recorded rather than rounded away. Immutable, so adding an edge means rebuilding; duplicate edges collapse and vertex ids must be dense. No edge weights and no shortest paths. |
