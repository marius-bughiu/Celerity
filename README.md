...
- For `string` keys, `StringFnV1AHasher` is the fast default for ASCII-dominated workloads (it folds only the low byte of each character). For keys with significant non-ASCII content, step up to `StringFnV1AFullHasher`, which folds the full UTF-16 character at the same FNV-1a cost class and so does not collide characters that differ only in their upper byte (e.g. `'A'` / `'Ł'`); for long or numerous keys where the 32-bit accumulator starts clustering, `StringFnV1A64Hasher` does the same full-character fold into a 64-bit state (effectively free on 64-bit platforms); escalate further to a strong-avalanche option when key distribution is clustered or adversarial — `StringMurmur3Hasher` (the `fmix32`-finalized MurmurHash3) for short keys, or `StringXxHash32Hasher` (xxHash32, which keeps four accumulators in flight) when keys are long enough for its throughput-oriented stripe loop to pull ahead.
