...
| `uint` / `ulong` | `UInt32WangNaiveHasher` (cheap XOR-fold) / `UInt64Murmur3Hasher` (`fmix64`) | Uniform / trusted keys → *drop* to `UInt32IdentityHasher` / `UInt64IdentityHasher` (the zero-work floor). `uint`: → `UInt32WangHasher` → `UInt32Murmur3Hasher`. `ulong`: drop to `UInt64WangHasher` / `UInt64WangNaiveHasher` when the `fmix64` multiplies cost more than they buy on uniform keys. |
