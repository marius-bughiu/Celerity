...
- `WaveletTree` — an **immutable** succinct index over a fixed sequence of `int` values that answers the questions a range fold cannot: the **`k`-th smallest value inside a positional window** (`Quantile`), how many values in that window fall in a **value band** (`RangeCount`), and the rank / select of a single value — each in `O(log σ)` over the *distinct* values, independent of how wide the window is. A `SegmentTree` folds a window down to one value and so structurally cannot reach the `k`-th; a `RankedSet` ranks a whole set with no positional window and no duplicates; the BCL has neither. The baseline is what a caller writes by hand — `Array.Sort` over a copy of the window for a quantile, a counting loop for a band — and at a million values with a 1% window the quantile is **~3,300× faster** and the band count **~40×**. Build-once, and it *loses* to a plain scan on a short window; both are measured in [the API reference](docs/api/collections.md#wavelettree).
