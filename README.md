...
- `PersistentVector<T>` — **immutable indexed sequence** over a **32-way bit-partitioned trie** with a tail buffer: every operation returns a new vector **sharing** all but `O(log32 n)` of the old one's storage, `this[int]` is at most seven array hops, and `Add` is amortized `O(1)`. The structure .NET does not ship, between the two that do — `ImmutableArray<T>` copies the whole array on every append, `ImmutableList<T>` is an AVL tree with a heap node per element and pays a pointer chase per level on **every read**. The one collection here that is safe for concurrent readers. Implements `IReadOnlyList<T>`.
