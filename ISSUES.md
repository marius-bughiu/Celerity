...
- **#9** — Implement `IReadOnlyDictionary<TKey, TValue>` (1.1.0). Status: `fixed in 1.1.0` — both `CelerityDictionary<TKey, TValue, THasher>` and `IntDictionary<TValue, THasher>` now implement `IReadOnlyDictionary<TKey, TValue?>` via thin explicit interface forwarders on top of the existing struct `KeyCollection` / `ValueCollection` views and struct `Enumerator`. The zero-allocation `foreach` fast path is preserved; the interface path boxes exactly once per `GetEnumerator()` call. The out-of-band default-key / zero-key entry is surfaced through every interface member.
...
- **#23** — `GetEnumerator()` on `IntSet` and `CeleritySet` (1.1.0). Status: `in-progress` — the dictionaries (`IntDictionary`, `CelerityDictionary`) ship a struct enumerator with `_version` mutation tracking via PRs #47 and #48, but the sets — `IntSet<THasher>` and `CeleritySet<T, THasher>` — currently expose no enumeration at all. There is no way to read the contents of a set short of asking `Contains` for every candidate value. This blocks (a) using sets as `foreach` sources without a wrapper, (b) implementing the post-1.1.0 `IReadOnlySet<T>` interface, and (c) implementing the `IEnumerable<T>` constructor to mirror dictionary issue #22. **Fix**: add a struct `Enumerator` (yielding the out-of-band zero / default entry first if present), a public `GetEnumerator()` returning that struct, an explicit `IEnumerable<T>` implementation forwarding to it, and `_version` increments on every entry-point mutation (`Add`, `TryAdd`, `Remove`, `Clear`). Mirror the regression-test surface from `IntDictionaryEnumerationTests` / `CelerityDictionaryEnumerationTests`. Done in two slices: `IntSet` first, `CeleritySet` next session.
