...
/// </para>
/// </remarks>
public struct StringSipHash13Hasher : IHashProvider<string>, IHashProvider64<string>, ISpanHashProvider
{
    ...
    /// <summary>
    /// Computes the SipHash-1-3 hash of the specified character span (using this type's
    /// fixed built-in key), xor-folded to a signed 32-bit result.
    /// </summary>
    /// <param name="key">The characters to hash.</param>
    /// <returns>
    /// The signed 32-bit, xor-folded SipHash-1-3 hash of <paramref name="key"/> — the same
    ...
    /// <summary>
    /// Computes the full 64-bit SipHash-1-3 (using this type's fixed built-in key) hash of the specified string over its
    /// native little-endian UTF-16 byte stream — the value <see cref="Hash(string)"/>
    /// xor-folds down to 32 bits.
    /// </summary>
    /// <param name="key">The string to hash. Must not be <c>null</c>.</param>
    /// <returns>The 64-bit SipHash-1-3 (using this type's fixed built-in key) hash of <paramref name="key"/>.</returns>
    /// <exception cref="ArgumentNullException">
    /// <paramref name="key"/> is <c>null</c>. Celerity dictionaries store the
    /// out-of-band <c>null</c>-key entry without calling the hasher, so this
    /// check only surfaces when the hasher is used directly or plugged into a
