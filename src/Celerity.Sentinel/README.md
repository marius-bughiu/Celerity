...
A rollup merges the lanes into one extra tracker that `Snapshot` keeps and reuses, so frequent rollups allocate only the report and the offender list read from each lane: the striped tracker costs `laneCount + 1` trackers of memory, not a fresh couple of megabytes per call.
