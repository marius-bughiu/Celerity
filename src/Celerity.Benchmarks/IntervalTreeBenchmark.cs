...
// what real calendars, IP tables and trace-span sets look like. The long spans are what keeps even the sorted
// baseline linear — they are the reason it cannot skip the prefix ahead of the query, only stop after it.
//
...
// deeper over each point the baseline's O(n) and the tree's cost converge. The mix here is a minority of spans
// covering ~1% of the domain each, which puts roughly 70 matches on a point at 100,000 intervals, where CI's
// same-runner A/B measures the point query at 129x the unsorted scan and 19x the sorted one. A shape ten times
// denser (spans covering a quarter of the domain, ~1,250 matches per point) was measured locally too, and the
// same query fell to 8.3x against the unsorted scan. The honest reading is that this type is for selective interval sets, which
