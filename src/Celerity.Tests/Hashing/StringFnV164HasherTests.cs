...
public class StringFnV164HasherTests
{
    ...
    /// <summary>
    /// The 64-bit state of the same independent FNV-1 reference used by
    /// <c>ReferenceFnv164Utf16</c>, before the xor-fold to 32 bits.
    /// </summary>
    private static ulong ReferenceFnv164Raw(string s)
    {
        ulong hash = OffsetBasis;
        foreach (byte b in Encoding.Unicode.GetBytes(s))
        {
    ...
    /// <summary>
    /// The hasher computes 64 bits of FNV-1 64-bit state internally and used to throw half of it
    ...
    public void Hash64_MatchesReferenceImplementation_AndFoldsToHash()
    {
        ...
        {
            ulong expected = ReferenceFnv164Raw(s);
