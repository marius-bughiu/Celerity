...
public class StringSdbmHasherTests
{
    private readonly StringSdbmHasher _hasher = new StringSdbmHasher();

    /// <summary>
    /// Independent reference: the classic sdbm hash over the BCL's little-endian
    /// UTF-16 encoding of the string. The hasher loops over <c>char</c>s and
    /// extracts the low/high byte by hand, expressing the step as
    /// <c>b + (hash &lt;&lt; 6) + (hash &lt;&lt; 16) - hash</c>; this reference goes through
    /// <see cref="Encoding.Unicode"/>, a flat byte loop, and the literal
    /// <c>* 65599</c>, so agreement pins the byte order, the seed, and the per-byte
    /// step against drift without sharing the hasher's implementation.
    /// </summary>
    private static int ReferenceSdbmUtf16(string s)
    {
        uint hash = 0u;
    ...
    // ── Exact-value anchor ────────────────────────────────────────────────────
    // The empty string folds no bytes, so the accumulator stays at the sdbm seed
    // constant 0.
    ...
    // ── Spec equivalence (sdbm over the UTF-16 byte stream) ────────────────────
    ...
    public void Hash_MatchesSdbmOverUtf16ByteStream(string input)
    {
        Assert.Equal(ReferenceSdbmUtf16(input), _hasher.Hash(input));
    }

    /// <summary>
    /// Independent value anchor for a single ASCII character, computed here by
    /// hand-running the sdbm step over the two UTF-16 bytes of <c>'a'</c>
    ...
    /// <c>(0 * 65599 + 0x61) * 65599 + 0x00</c>.
    /// </summary>
    [Fact]
    public void Hash_SingleAsciiChar_MatchesHandComputedSdbm()
    {
        uint expected = ((0u * 65599u + 0x61u) * 65599u) + 0x00u;
    ...
    public void Hash_IsDeterministic_AcrossInstances()
    {
        ...
        int a = new StringSdbmHasher().Hash(value);
        int b = new StringSdbmHasher().Hash(value);
        Assert.Equal(a, b);
    }
    ...
    public void Hash_DistinguishesCharactersDifferingOnlyInUpperByte()
    {
        ...
        // single-character strings; StringSdbmHasher folds the full 16-bit
    ...
    public void Hash_DistinguishesMultibyteStringsThatLowByteFnv1aCollides()
    {
        ...
        // upper bytes: the low-byte FNV-1a collides, the full-width sdbm does not.
        var lowByte = new StringFnV1AHasher();
    ...
    [Fact]
    public void StringSdbmHasher_CanDriveCelerityDictionary()
    {
        var dict = new CelerityDictionary<string, int, StringSdbmHasher>();
    ...
    [Fact]
    public void StringSdbmHasher_KeepsUpperByteDistinctKeysSeparate()
    {
    ...
    [Fact]
    public void StringSdbmHasher_HandlesNullKeyOutOfBand()
    {
    ...
    [Fact]
    public void StringSdbmHasher_CanDriveCeleritySet()
    {
        var set = new CeleritySet<string, StringSdbmHasher>();
