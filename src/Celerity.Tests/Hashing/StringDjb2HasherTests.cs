...
public class StringDjb2HasherTests
{
    private readonly StringDjb2Hasher _hasher = new StringDjb2Hasher();

    /// <summary>
    /// Independent reference: Daniel J. Bernstein's djb2 hash over the BCL's
    ...
    /// <c>* 33</c>, so agreement pins the byte order, the seed, and the per-byte
    ...
    // ── Spec equivalence (djb2 over the UTF-16 byte stream) ────────────────────
    ...
    public void Hash_MatchesDjb2OverUtf16ByteStream(string input)
    {
        Assert.Equal(ReferenceDjb2Utf16(input), _hasher.Hash(input));
    }

    /// <summary>
    /// Independent value anchor for a single ASCII character, computed here by
    /// hand-running the djb2 step over the two UTF-16 bytes of <c>'a'</c>
    ...
    /// </summary>
    [Fact]
    public void Hash_SingleAsciiChar_MatchesHandComputedDjb2()
    {
    ...
    public void Hash_IsDeterministic_AcrossInstances()
    {
        ...
        int a = new StringDjb2Hasher().Hash(value);
        int b = new StringDjb2Hasher().Hash(value);
        Assert.Equal(a, b);
    }
    ...
    public void Hash_DistinguishesCharactersDifferingOnlyInUpperByte()
    {
        ...
        // single-character strings; StringDjb2Hasher folds the full 16-bit
    ...
    public void Hash_DistinguishesMultibyteStringsThatLowByteFnv1aCollides()
    {
        ...
        // upper bytes: the low-byte FNV-1a collides, the full-width djb2 does not.
        var lowByte = new StringFnV1AHasher();
    ...
    [Fact]
    public void StringDjb2Hasher_CanDriveCelerityDictionary()
    {
        var dict = new CelerityDictionary<string, int, StringDjb2Hasher>();
    ...
    [Fact]
    public void StringDjb2Hasher_KeepsUpperByteDistinctKeysSeparate()
    {
    ...
    [Fact]
    public void StringDjb2Hasher_HandlesNullKeyOutOfBand()
    {
    ...
    [Fact]
    public void StringDjb2Hasher_CanDriveCeleritySet()
    {
        var set = new CeleritySet<string, StringDjb2Hasher>();
