...
public class IEnumerableConstructorTests
{
    ...
    // ──────────────────────────────────────────────────────────────
    //  HashCachingDictionary — source argument validation
    // ──────────────────────────────────────────────────────────────

    [Fact]
    public void HashCachingDictionary_ShouldThrow_WhenSourceIsNull()
    {
        IEnumerable<KeyValuePair<int, string>>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new HashCachingDictionary<int, string, Int32WangNaiveHasher>(source!));

        Assert.Equal("source", ex.ParamName);
    }

    [Fact]
    public void HashCachingDictionary_ShouldThrow_OnDuplicateKeysInSource()
    {
        var source = new[]
        {
            new KeyValuePair<string, int>("a", 1),
        ...
        var ex = Assert.Throws<ArgumentException>(() =>
            new HashCachingDictionary<string, int, StringFnV1AHasher>(source));
        Assert.Contains("a", ex.Message);
    }

    [Fact]
    public void HashCachingDictionary_ShouldStillValidate_LoadFactor_WhenConstructedFromSource()
    {
        var source = new[] { new KeyValuePair<int, int>(1, 1) };

        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new HashCachingDictionary<int, int, Int32WangNaiveHasher>(source, loadFactor: 1f));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new HashCachingDictionary<int, int, Int32WangNaiveHasher>(source, loadFactor: 0f));
    }

    // ──────────────────────────────────────────────────────────────
    //  HashCachingDictionary — happy path
    // ──────────────────────────────────────────────────────────────

    [Fact]
    public void HashCachingDictionary_ShouldSupportEmptySource()
    {
    ...
    [Fact]
    public void HashCachingDictionary_ShouldCopyAllEntries_FromArraySource()
    {
        var source = new[]
        {
            new KeyValuePair<string, int>("alpha", 1),
        ...
        var map = new HashCachingDictionary<string, int, StringFnV1AHasher>(source);
    ...
    [Fact]
    public void HashCachingDictionary_ShouldCopyAllEntries_FromNonCollectionEnumerable()
    {
        IEnumerable<KeyValuePair<int, string>> NonCollectionSource()
        {
            yield return new KeyValuePair<int, string>(1, "one");
        ...
        var map = new HashCachingDictionary<int, string, Int32WangNaiveHasher>(NonCollectionSource());
    ...
    [Fact]
    public void HashCachingDictionary_ShouldCaptureDefaultIntKey_FromSource()
    {
        var source = new[]
        {
            new KeyValuePair<int, string>(0, "zero"),
        ...
        var map = new HashCachingDictionary<int, string, Int32WangNaiveHasher>(source);
    ...
    [Fact]
    public void HashCachingDictionary_ShouldCaptureNullStringKey_FromSource()
    {
        var source = new[]
        {
            new KeyValuePair<string, int>(null!, 0),
    ...
    [Fact]
    public void HashCachingDictionary_ShouldDetectDuplicateDefaultKey()
    {
        var source = new[]
        {
            new KeyValuePair<int, string>(0, "zero"),
    ...
    [Fact]
    public void HashCachingDictionary_ShouldCopy_LargeSource_WithoutDataLoss()
    {
        var source = Enumerable.Range(1, 500)
            .Select(i => new KeyValuePair<string, int>($"key-{i}", i))
            .ToArray();
    ...
    [Fact]
    public void HashCachingDictionary_ShouldBeIndependent_OfSourceAfterConstruction()
    {
        var source = new List<KeyValuePair<int, string>>
        {
            new(1, "one"),
    ...
    [Fact]
    public void HashCachingDictionary_ShouldCopy_FromAnotherHashCachingDictionary()
    {
        var original = new HashCachingDictionary<string, int, StringFnV1AHasher>();
        original.Add("alpha", 1);
        original.Add("beta", 2);

        var copy = new HashCachingDictionary<string, int, StringFnV1AHasher>(
            original.Select(kvp => new KeyValuePair<string, int>(kvp.Key, kvp.Value)));
