...
// Dedicated tests for HashCachingSet, mirroring CeleritySetTests / RobinHoodSetTests
// and the HashCachingDictionary dedicated tests. HashCachingSet is a drop-in peer of
// CeleritySet that keeps a dense side array of cached hash fingerprints, so it must
// satisfy the same functional contract; the fingerprint-specific stress
// (collisions on the cached fingerprint, backward-shift deletion driven by the
// cached natural slot) lives in HashCachingSetCollisionTests.
public class HashCachingSetTests
{
    [Fact]
    ...
    {
        var set = new HashCachingSet<int, Int32WangNaiveHasher>(16, loadFactor: 0.5f);
        for (int i = 1; i <= 40; i++)
    ...
    {
        var set = new HashCachingSet<Guid, GuidIdentityHasher>();
        set.Add(Guid.Empty);
    ...
    {
        var set = new HashCachingSet<int, Int32WangNaiveHasher>(16);
        for (int i = 1; i <= 100; i++)
            set.Add(i);
    ...
    {
        var set = new HashCachingSet<string, StringFnV1AHasher>();
