...
public class BulkConstructorNoResizeTests
{
    ...
    [Fact]
    public void HashCachingDictionary_BulkConstruct_FromKnownCount_DoesNotResize()
    {
        KeyValuePair<string, int>[] src = StringPairs();
        _hashCallCount = 0;

        var map = new HashCachingDictionary<string, int, CountingStringHasher>(src);
    ...
    [Fact]
    public void RobinHoodSet_BulkConstruct_FromKnownCount_DoesNotResize()
    {
        string[] src = Enumerable.Range(1, N).Select(i => $"v{i}").ToArray();
        _hashCallCount = 0;

        var set = new RobinHoodSet<string, CountingStringHasher>(src);
    ...
    [Fact]
    public void HashCachingSet_BulkConstruct_FromKnownCount_DoesNotResize()
    {
        string[] src = Enumerable.Range(1, N).Select(i => $"v{i}").ToArray();
        _hashCallCount = 0;

        var set = new HashCachingSet<string, CountingStringHasher>(src);
    ...
    [Fact]
    public void PooledCeleritySet_BulkConstruct_FromKnownCount_DoesNotResize()
    {
        string[] src = Enumerable.Range(1, N).Select(i => $"v{i}").ToArray();
        _hashCallCount = 0;

        using var set = new PooledCeleritySet<string, CountingStringHasher>(src);
