...
// Enumeration tests for SwissSet, mirroring CeleritySetEnumerationTests. The
...
public class SwissSetEnumerationTests
{
    [Fact]
    ...
    {
        var set = new SwissSet<int, Int32WangNaiveHasher>(capacity: 16);
        for (int i = 1; i <= 200; i++)
            set.Add(i);
    ...
    {
        var set = new SwissSet<int, Int32Murmur3Hasher>();
        set.Add(0);
        for (int i = 1; i <= 25; i++)
            set.Add(i);
