...
// Milestone 1.1.0 — issue #23: GetEnumerator on IntSet.
//
// These tests pin down the observable behavior of the new struct enumerator on
// IntSet<THasher>:
...
//   IntSet's contents by set equality.
...
// - The IEnumerable<int> path behaves identically to the struct path
//   (it boxes the enumerator, but the set semantics line up).
public class IntSetEnumerationTests
{
    [Fact]
    ...
    {
        // The contract is that the out-of-band zero entry is yielded first.
        // Subsequent ordering is unspecified, but the *first* element when zero
        // is present must be zero.
        var set = new IntSet();
        set.Add(11);
        set.Add(22);
        set.Add(33);
    ...
    public void MoveNext_ShouldThrow_WhenZeroInsertedDuringEnumeration()
    {
        ...
        set.Add(0); // zero-entry insert goes through the out-of-band slot.
    ...
    public void MoveNext_ShouldThrow_AfterZeroRemoveDuringEnumeration()
    {
        ...
        Assert.True(set.Remove(0)); // out-of-band zero removal
    ...
    public void TryAdd_ShouldNotBumpVersion_WhenItemAlreadyPresent()
    {
        ...
        // We don't assert on advanced specifically because zero may or may not
        // have been emitted first; the contract is just that no exception fires.
        _ = advanced;
    }
    ...
    public void Enumerator_ShouldReturnDefault_AfterEnumerationIsExhausted()
    {
        ...
        Assert.False(enumerator.MoveNext());
        // After exhaustion the enumerator's Current is reset to default(int) — 0.
        // (BCL Dictionary returns default; we match that contract.)
        Assert.Equal(0, enumerator.Current);
    }
    ...
    {
        // Iterating through the IEnumerable<int> interface boxes the struct
    ...
    {
        // Drive the enumerator on the open-generic IntSet<THasher> path to
        // confirm the surface works for callers that pick a custom hasher.
        var set = new IntSet<Celerity.Hashing.Int32Murmur3Hasher>();
        set.Add(0);
        for (int i = 1; i <= 25; i++)
            set.Add(i);
    ...
    {
        // The IEnumerable<int> conformance unlocks LINQ. Probe the most common
