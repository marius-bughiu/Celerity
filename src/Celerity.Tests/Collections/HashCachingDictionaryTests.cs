...
// Dedicated tests for HashCachingDictionary (issue #65), mirroring
// CelerityDictionaryTests / SwissDictionaryTests. HashCachingDictionary is a
// drop-in peer of CelerityDictionary that keeps a parallel array of cached hash
// fingerprints (struct-of-arrays layout) and short-circuits the equality check
// on probe, so it must satisfy the same functional contract; the algorithm-
// specific stress (fingerprint collisions, probe scans, resize) is in
// HashCachingDictionaryCollisionTests.
public class HashCachingDictionaryTests
{
    [Fact]
    ...
    {
        var map = new HashCachingDictionary<int, int, Int32WangNaiveHasher>(16, loadFactor: 0.5f);
        for (int i = 1; i <= 40; i++)
    ...
    {
        var map = new HashCachingDictionary<Guid, string, GuidIdentityHasher>();
