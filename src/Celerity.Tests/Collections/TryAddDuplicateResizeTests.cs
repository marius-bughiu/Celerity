...
public class TryAddDuplicateResizeTests
{
    ...
    // HashCachingSet's default table is 16 slots, so its default-capacity threshold
    // is (int)(16 * 0.75) = 12 — the same at-threshold boundary as SwissSet.

    [Fact]
    public void HashCachingSet_TryAdd_DuplicateAtThreshold_KeepsEnumeratorValid()
    {
        var set = new HashCachingSet<int, Celerity.Hashing.Int32WangNaiveHasher>(capacity: 16);
        for (int i = 1; i <= 12; i++)
            set.Add(i); // Count == 12 == threshold
    ...
    [Fact]
    public void HashCachingSet_TryAdd_NewItemAtThreshold_InvalidatesEnumerator()
    {
        var set = new HashCachingSet<int, Celerity.Hashing.Int32WangNaiveHasher>(capacity: 16);
        for (int i = 1; i <= 12; i++)
            set.Add(i); // Count == 12 == threshold
