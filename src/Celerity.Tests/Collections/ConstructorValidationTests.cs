...
public class ConstructorValidationTests
{
    ...
    // ──────────────────────────────────────────────────────────────
    //  SwissDictionary — loadFactor validation
    // ──────────────────────────────────────────────────────────────
    ...
    [InlineData(-0.5f)]
    [InlineData(-1f)]
    public void SwissDictionary_ShouldThrow_WhenLoadFactorIsZeroOrNegative(float loadFactor)
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SwissDictionary<int, int, Int32WangNaiveHasher>(capacity: 16, loadFactor: loadFactor));
    }
    ...
    [InlineData(1.5f)]
    [InlineData(2f)]
    public void SwissDictionary_ShouldThrow_WhenLoadFactorIsOneOrGreater(float loadFactor)
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SwissDictionary<int, int, Int32WangNaiveHasher>(capacity: 16, loadFactor: loadFactor));
    }
    ...
    [Fact]
    public void SwissDictionary_ShouldThrow_WhenCapacityIsNegative()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SwissDictionary<int, int, Int32WangNaiveHasher>(capacity: -1));
    }

    [Fact]
    public void SwissDictionary_ShouldAcceptZeroCapacity()
    {
        var map = new SwissDictionary<int, int, Int32WangNaiveHasher>(capacity: 0);
        map[42] = 100;
        Assert.Equal(100, map[42]);
    }
    ...
    [InlineData(0.75f)]
    [InlineData(0.99f)]
    public void SwissDictionary_ShouldAcceptValidLoadFactor(float loadFactor)
    {
        var map = new SwissDictionary<int, int, Int32WangNaiveHasher>(capacity: 16, loadFactor: loadFactor);
        map[1] = 10;
        Assert.Equal(10, map[1]);
    }
    ...
    // ──────────────────────────────────────────────────────────────
    //  CelerityMultiSet — same capacity / loadFactor validation
    // ──────────────────────────────────────────────────────────────
    ...
    [InlineData(-0.5f)]
    [InlineData(-1f)]
    public void CelerityMultiSet_ShouldThrow_WhenLoadFactorIsZeroOrNegative(float loadFactor)
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new CelerityMultiSet<int, Int32WangNaiveHasher>(capacity: 16, loadFactor: loadFactor));
    }
    ...
    [InlineData(1.5f)]
    [InlineData(2f)]
    public void CelerityMultiSet_ShouldThrow_WhenLoadFactorIsOneOrGreater(float loadFactor)
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new CelerityMultiSet<int, Int32WangNaiveHasher>(capacity: 16, loadFactor: loadFactor));
    }
    ...
    [Fact]
    public void CelerityMultiSet_ShouldThrow_WhenCapacityIsNegative()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new CelerityMultiSet<int, Int32WangNaiveHasher>(capacity: -1));
    }

    [Fact]
    public void CelerityMultiSet_ShouldAcceptZeroCapacity()
    {
        var set = new CelerityMultiSet<int, Int32WangNaiveHasher>(capacity: 0);
        set.Add(42, 100);
        Assert.Equal(100, set[42]);
    }
    ...
    [InlineData(0.75f)]
    [InlineData(0.99f)]
    public void CelerityMultiSet_ShouldAcceptValidLoadFactor(float loadFactor)
    {
        var set = new CelerityMultiSet<int, Int32WangNaiveHasher>(capacity: 16, loadFactor: loadFactor);
        set.Add(1, 10);
