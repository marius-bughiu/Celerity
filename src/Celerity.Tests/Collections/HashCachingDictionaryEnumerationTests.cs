...
// Issue #65 — Keys / Values / GetEnumerator on HashCachingDictionary.
//
// Mirror of CelerityDictionaryEnumerationTests for HashCachingDictionary.
...
// and mid-enumeration mutation detection. Enumeration walks the cached
// fingerprint array for occupancy, so these tests also pin that the
// fingerprint metadata stays in sync with the key/value arrays across
// insert, remove, clear, and resize.
public class HashCachingDictionaryEnumerationTests
{
    [Fact]
    ...
    public void GetEnumerator_ShouldSurviveResize()
    {
        ...
        var map = new HashCachingDictionary<int, int, Int32WangNaiveHasher>(capacity: 4);
        for (int i = 1; i <= 200; i++)
            map[i] = -i;
