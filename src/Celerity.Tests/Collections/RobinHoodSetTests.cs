...
// Dedicated tests for RobinHoodSet, mirroring CeleritySetTests / SwissSetTests and
// the RobinHoodDictionary dedicated tests. RobinHoodSet is a drop-in peer of
// CeleritySet that resolves collisions with Robin Hood open addressing, so it must
// satisfy the same functional contract; the algorithm-specific stress
// (displacement, backward-shift-with-distance-decrement deletion) lives in
// RobinHoodSetCollisionTests.
public class RobinHoodSetTests
{
    [Fact]
    ...
    {
        var set = new RobinHoodSet<int, Int32WangNaiveHasher>(16, loadFactor: 0.5f);
        for (int i = 1; i <= 40; i++)
    ...
    {
        var set = new RobinHoodSet<Guid, GuidIdentityHasher>();
        set.Add(Guid.Empty);
    ...
    {
        var set = new RobinHoodSet<int, Int32WangNaiveHasher>(16);
        for (int i = 1; i <= 100; i++)
            set.Add(i);
