...
public class IEnumerableConstructorNullPriorityTests
{
    ...
    [Fact]
    public void SwissDictionary_NullSourceWithInvalidLoadFactor_ShouldThrow_ArgumentNullException()
    {
        IEnumerable<KeyValuePair<int, string>>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new SwissDictionary<int, string, Int32WangNaiveHasher>(source!, capacity: 16, loadFactor: 2.0f));

        Assert.Equal("source", ex.ParamName);
    }

    [Fact]
    public void HashCachingDictionary_NullSourceWithInvalidLoadFactor_ShouldThrow_ArgumentNullException()
    {
        IEnumerable<KeyValuePair<int, string>>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new HashCachingDictionary<int, string, Int32WangNaiveHasher>(source!, capacity: 16, loadFactor: 2.0f));
    ...
    [Fact]
    public void PooledCelerityDictionary_NullSourceWithInvalidLoadFactor_ShouldThrow_ArgumentNullException()
    {
        IEnumerable<KeyValuePair<int, string>>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new PooledCelerityDictionary<int, string, Int32WangNaiveHasher>(source!, capacity: 16, loadFactor: 2.0f));
    ...
    [Fact]
    public void SwissSet_NullSourceWithInvalidLoadFactor_ShouldThrow_ArgumentNullException()
    {
        IEnumerable<string>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new SwissSet<string, StringFnV1AHasher>(source!, capacity: 16, loadFactor: 2.0f));

        Assert.Equal("source", ex.ParamName);
    }

    [Fact]
    public void RobinHoodSet_NullSourceWithInvalidLoadFactor_ShouldThrow_ArgumentNullException()
    {
        IEnumerable<string>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new RobinHoodSet<string, StringFnV1AHasher>(source!, capacity: 16, loadFactor: 2.0f));

        Assert.Equal("source", ex.ParamName);
    }

    [Fact]
    public void HashCachingSet_NullSourceWithInvalidLoadFactor_ShouldThrow_ArgumentNullException()
    {
        IEnumerable<string>? source = null;

        var ex = Assert.Throws<ArgumentNullException>(() =>
            new HashCachingSet<string, StringFnV1AHasher>(source!, capacity: 16, loadFactor: 2.0f));
